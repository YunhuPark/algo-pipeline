...
def _is_listicle_topic(topic: str) -> bool:
    """N가지/N선/TOP N 형식 리스트형 주제 감지"""
    return bool(_re.search(r'\d+\s*(?:가지|선|개|위)', topic)) or \
           bool(_re.search(r'(?:top|best)\s*\d+', topic.lower()))


def _get_listicle_n(topic: str) -> int:
    m = _re.search(r'(\d+)\s*(?:가지|선|개|위)', topic)
...
없는 내용 추가·상상 절대 금지{listicle_override}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
【카드 5 — 알고의 시선 (Insight / Impact)】
  ...
  title: ≤20자. 이 기사/기술이 독자 삶에 만드는 구체적 변화를 한 줄로. 질문형 또는 단언형.
         ✓ "이제 코딩 없이 배포 가능" / "직장인 야근이 줄어드는 이유"
         ❌ "통찰이 있는가?" / "우리에게 시사하는 바" / "미래를 바꿀 기술"
  body:  반드시 아래 2줄 형식만 (3줄 이상 즉시 탈락):
         줄1: "💡 알고의 한 줄 요약: [이 기사의 핵심 변화를 25자 이내로]"
         줄2: 독자가 이 뉴스로 달라지는 점 1문장, ≤25자. 수치·직군·행동 포함.
         ✓ 예: "💡 알고의 한 줄 요약: Codex로 재무팀도 코딩 없이 자동화\n반복 업무 2시간 → 5분"
         ✓ 예: "💡 알고의 한 줄 요약: GPT-5, 기존 4o보다 정확도 40% 향상\n직장인·개발자 모두 지금 갈아타는 이유"
         ❌ 금지: 3줄 이상 / "다양한 분야" / "혁신적" / 추상적 감상 / 기사에 없는 예측
  accent: 핵심 변화 키워드 (수치 또는 직군명 포함)
...
짧게 끊어서 써. 한 문장에 아이디어 하나. AI 냄새 나는 표현("이를 통해", "혁신적인", "다양한")은 즉시 교체.{angle_structure}{feedback_section}"""
...
카드 구조 (기존 6장 구조 무시하고 아래로 대체):
  카드 1 (cover): title에 "N가지" 숫자 포함 필수.
                  body: ≤25자. 아래 형식 택1 (클리셰 금지):
                  [형식A] 힌트형 — "이 중 3개는 지금 무료" / "이거 모르면 솔직히 손해"
                  [형식B] FOMO형 — "이 중 몇 개나 써봤어?" / "나만 몰랐던 거였나?"
                  ❌ 절대 금지: "저장해두면 하나씩 써먹을 수 있어요" (너무 일반적 — 즉시 탈락)
...
def run(
    ...
    # ── 리스트형 주제 감지 — 기사 제약 해제 후 GPT 지식 기반 생성 ──
    is_listicle = _is_listicle_topic(topic)
    if is_listicle:
        list_n = _get_listicle_n(topic)
        # article_title을 원래 주제로 교체 → GPT가 기사 제목에 묶이지 않도록
        article_title = topic
        _trend_ctx_parts = []
        if article_facts.core_tension:
            _trend_ctx_parts.append(f"현재 AI 트렌드: {article_facts.core_tension}")
        for _f in article_facts.facts[:3]:
    ...
    invoke_kwargs = {
        ...
        "listicle_override": listicle_override,
    }
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # 알고의 한 줄 요약 누락 검사 (카드 5, 리스트형 모드 제외)
            insight_card = next((s for s in result.slides if s.slide_type == "content" and s.slide_number == num_cards - 1), None)
            if not is_listicle and insight_card and "💡 알고의 한 줄 요약" not in (insight_card.body or ""):
