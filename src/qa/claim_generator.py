...
3. 숫자가 포함된 문장은 반드시 numerical type을 사용하고, numbers 배열에 해당 숫자를 명시하십시오.
   numbers 배열에 넣는 숫자는 claim_text 문장 안에도 그대로 적어야 합니다 — 카드 상단
   강조 수치로만 쓰고 본문 문장에서는 빼면 안 됩니다. 예: "그의 순자산은 192억 달러입니다"
   처럼 숫자와 그 의미를 한 문장 안에서 함께 밝히십시오.
...
8-1. 각 Claim은 **다른 Claim에는 없는 구체적 정보**를 하나 이상 담아야 합니다. 가격·수치·날짜·사양·조건·비교 대상 중 원문에 실제로 적힌 것을 쓰십시오.
     같은 사건을 관점만 바꿔 되풀이하면 안 됩니다. 예를 들어 "A사가 X를 공개했다" / "X 공개로 B사 독점이 끝났다" / "X로 시장에 변화가 예상된다"는 정보량이 같으므로 한 장으로 취급하고, 나머지 카드는 가격·출시일·사양·이전 제품과의 차이처럼 원문이 알려주는 다른 사실로 채우십시오.
